...
| 端口 | LDAP 端口 | `389` |
| 管理员账号 | 有 OU 写入权限的 AD 账号 | `administrator@corp.realman-robot.com` |
| 密码 | 管理员密码 | `********` |
| Base DN | 最外层 OU 路径 | `OU=Users,OU=REALMAN,DC=corp,DC=realman-robot,DC=com` |

#### 同步策略

| 字段 | 说明 | 默认值 |
|------|------|--------|
| 初始密码 | 新用户默认密码 | `Realman@2026` |
...
钉钉组织树                         AD OU 树
───────────                      ──────────
睿尔曼智能 (根)        ───→      OU=睿尔曼智能,OU=Users,OU=REALMAN,DC=...
├── 研发部              ───→      ├── OU=研发部
│   ├── 前端组          ───→      │   └── OU=前端组
│   └── 后端组          ───→      │       └── OU=后端组
├── 销售部              ───→      ├── OU=销售部
...
<p align="center">
  <sub>Built for Realman Intelligent Technology · 睿尔曼智能科技</sub>
</p>
